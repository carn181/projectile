...
void text_update(SDL_Renderer *renderer, struct font *font, const char *text,
                 int x, int y) {
...
void clearscene(struct program *p) {
  SDL_SetRenderDrawColor(p->renderer, p->bgcolor.r, p->bgcolor.g, p->bgcolor.b,
                         255);
  SDL_RenderClear(p->renderer);
}

float degree2rad(float theta) { return theta * (M_PI / 180); }

float projectilefunc(float x, float theta, float u, float g) {
  double t = tan(theta);
  double c = cos(theta);
  return ((x * t) - (g * pow(x, 2) / (2 * pow(u, 2) * pow(c, 2))));
}
...
void drawprojectile(struct program *p) {
  ...
  for (float i = 0; i < 640; i += 0.5) {
    SDL_RenderDrawPointF(p->renderer, i,
                         480 -
                             projectilefunc(i, degree2rad(p->projectile.angle),
